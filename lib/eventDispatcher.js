...
EventDispatcher.prototype = {
  ...
  dispatch: function (evt, callback) {
    ...
    var sagaParts = this.tree.getSagaPart(target);
    ...
    var commandsToDispatch = [];
    async.eachSeries(sagaParts, function (saga, callback) {
      saga.handle(evt, function (err, cmds) {
        if (cmds && _.isArray(cmds) && cmds.length > 0) {
          commandsToDispatch = commandsToDispatch.concat(cmds)
        }
        callback(err);
      })
    }, function (err) {
      callback(err, commandsToDispatch);
    });
  }
